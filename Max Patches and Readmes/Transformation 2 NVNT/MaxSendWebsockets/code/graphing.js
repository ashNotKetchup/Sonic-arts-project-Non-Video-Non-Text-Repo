...
class MyGraph{
    ...
    //Change strength and distance to fit different display sizes (greater=more spread)
        ...
        .on("tick", ()=>{
            ...
            */
            this.linkLabel.attr("x", d=> ((d.source.x+d.target.x)/2)+((d.target.x-d.source.x)/1000))
            .attr("y", d=> ((d.source.y+d.target.y)/2)+((d.target.x-d.source.x)/1000));
            }
        );
    ...
    this.svgPlus = Object.assign(this.svg.node(), {
                update(ogGraph, {nodes, links}) {
                  ...
                  */
                    ...
                    .attr("stroke-width", d => 10*(d.weight**1.3))
